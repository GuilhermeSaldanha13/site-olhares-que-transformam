...
export function Inscricao() {
    ...
    function onChange(e) {
        ...
        if (name === "cpf") {
            setValues((v) => ({ ...v, cpf: value.replace(/\D/g, "").slice(0, 11) }))
            return
        }
        if (name === "whatsapp") {
            setValues((v) => ({ ...v, whatsapp: value.replace(/\D/g, "").slice(0, 13) }))
            return
        }
                ...
                {status === "ok" ? (
                    <>
                        ...
                        <div className="mx-auto mt-8 max-w-xl space-y-6">
                            ...
                            <a href={WHATSAPP_GROUP_URL} target="_blank" rel="noopener noreferrer" className="inline-flex items-center justify-center gap-3 rounded-lg bg-[#25D366] px-8 py-4 text-lg font-semibold text-white shadow-lg transition-all hover:bg-[#22c35e] hover:scale-105">
                    ...
                    <>
                        ...
                        <form onSubmit={onSubmit} className="mx-auto mt-8 grid max-w-xl gap-3 text-left">
                            ...
                            <label className="flex flex-col">
                                <span className="text-sm">Profissao (opcional)</span>
                            ...
                            <label className="flex flex-col">
                                <span className="text-sm">Motivacao (opcional)</span>
                            ...
                            <div className="mt-4 flex items-center gap-3">
                                ...
                                {status && status.startsWith("error") && <span className="text-sm text-destructive">Erro ao enviar</span>}
                            </div>
                        </form>
                    </>
                )}
